...
-- Comprehensive deletion of all client and provider profiles and associated data
-- This preserves admin users and platform configuration
...
-- Store user IDs that will be deleted for auth table cleanup
CREATE TEMP TABLE users_to_delete AS 
SELECT id FROM users WHERE role IN ('client', 'provider');

-- Finally, delete the user profiles themselves
DELETE FROM users WHERE role IN ('client', 'provider');

-- Delete from auth.users table (this removes the authentication records)
